...
@section('content')
<div class="mb-6">
    <h1 class="font-display text-2xl font-semibold text-forest-950">Edit Guru</h1>
    <p class="text-sm text-forest-700/70 mt-1">{{ $teacher->name }}</p>
</div>

<form method="POST" action="{{ route('teachers.update', $teacher) }}" class="max-w-2xl bg-paper-50 border border-forest-100 rounded-md p-6 space-y-5">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
            ...
            <input type="text" id="nuptk" name="nuptk" value="{{ old('nuptk', $teacher->nuptk) }}" maxlength="16"
        ...
        <div>
            ...
            <input type="text" id="nip" name="nip" value="{{ old('nip', $teacher->nip) }}" maxlength="18"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
            ...
            <select id="subject_id" name="subject_id" class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none @error('subject_id') border-red-400 @enderror">
                ...
                @foreach ($subjects as $subject)
                    <option value="{{ $subject->id }}" @selected((string) old('subject_id', $teacher->subject_id) === (string) $subject->id)>{{ $subject->name }}</option>
                @endforeach
            </select>
            <label for="subject_text" class="block text-xs text-forest-600 mt-2 mb-1">Mapel lain (catatan bebas, opsional)</label>
            <input type="text" id="subject_text" name="subject_text" value="{{ old('subject_text', $teacher->subject_text) }}" maxlength="100"
                class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none">
            @error('subject_id')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <select id="employment_status" name="employment_status" class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none @error('employment_status') border-red-400 @enderror">
                <option value="">-- Pilih --</option>
                <option value="gty" @selected(old('employment_status', $teacher->employment_status) === 'gty')>GTY (Guru Tetap Yayasan)</option>
                <option value="ptt" @selected(old('employment_status', $teacher->employment_status) === 'ptt')>PTT (Guru Tidak Tetap)</option>
                <option value="asn" @selected(old('employment_status', $teacher->employment_status) === 'asn')>ASN</option>
            </select>
    ...
    <div>
        ...
        <textarea id="address" name="address" rows="2" class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none">{{ old('address', $teacher->address) }}</textarea>
    </div>

    <div>
        <label for="phone" class="block text-sm font-medium text-forest-800 mb-1">No. HP / Telepon</label>
        <input type="text" id="phone" name="phone" value="{{ old('phone', $teacher->phone) }}" maxlength="20"
            class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none">
    </div>
