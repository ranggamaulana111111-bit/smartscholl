...
@section('content')
<div class="mb-6">
    <h1 class="font-display text-2xl font-semibold text-text">Edit Jurnal KBM</h1>
</div>
<form method="POST" action="{{ route('journals.update', $journal) }}" class="card max-w-2xl p-6 space-y-5">
    @csrf
    @method('PUT')
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
            <label class="label">Tanggal</label>
            <input type="date" name="date" value="{{ old('date', $journal->date?->toDateString()) }}" max="{{ now()->toDateString() }}"
    ...
    <div>
        ...
        <input type="text" name="topic" value="{{ old('topic', $journal->topic) }}" maxlength="255"
