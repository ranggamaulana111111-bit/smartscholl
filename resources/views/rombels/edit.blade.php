...
@section('content')
<div class="mb-6">
    <h1 class="font-display text-2xl font-semibold text-forest-950">Edit Rombel</h1>
    <p class="text-sm text-forest-700/70 mt-1">{{ $rombel->name }} ({{ $rombel->grade_level }})</p>
</div>

<form method="POST" action="{{ route('rombels.update', $rombel) }}" class="max-w-2xl bg-paper-50 border border-forest-100 rounded-md p-6 space-y-5">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
            ...
            <select id="academic_year_id" name="academic_year_id" class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none @error('academic_year_id') border-red-400 @enderror">
                ...
                @foreach($academicYears as $year)
                    <option value="{{ $year->id }}" @selected(old('academic_year_id', $rombel->academic_year_id) == $year->id)>
        ...
        <div>
            ...
            <input type="text" id="grade_level" name="grade_level" value="{{ old('grade_level', $rombel->grade_level) }}" maxlength="10" placeholder="contoh: X"
    ...
    <div>
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $rombel->name) }}" maxlength="50" placeholder="contoh: X-1"
    ...
    <div>
        ...
        <select id="homeroom_teacher_id" name="homeroom_teacher_id" class="w-full px-3 py-2 rounded-md border border-forest-200 bg-paper-50 text-forest-950 text-sm focus:border-gold-500 outline-none @error('homeroom_teacher_id') border-red-400 @enderror">
            ...
            @foreach($teachers as $teacher)
                <option value="{{ $teacher->id }}" @selected(old('homeroom_teacher_id', $rombel->homeroom_teacher_id) == $teacher->id)>{{ $teacher->name }}</option>
            @endforeach
        </select>
